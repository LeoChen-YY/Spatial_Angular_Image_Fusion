...
% 2.5.0 準備插值所需的座標 ---
% 假設 N 是總角度數，idx_sampled 是已採樣的角度索引
idx_all = 1:N;
idx_sampled = find(any(reshape(full(P_A_vec), [M, N]), 1)); % 找出有採樣的角度列

% 2.5.1 進行插值 (對每一行 detector 進行插值) ---
s_A_mat = reshape(full(s_A), [M, N]);
s_interp_mat = zeros(M, N);

for i = 1:M
    % 使用線性插值填補缺失角度，邊界外使用 'extrap' 避免 NaN
    s_interp_mat(i, :) = interp1(idx_sampled, s_A_mat(i, idx_sampled), idx_all, 'linear', 'extrap');
end

% 2.5.2 將插值結果作為 s_k 的初始值 ---
s_init = s_interp_mat(:);
