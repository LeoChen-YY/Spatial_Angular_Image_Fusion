%% CT Sinogram Fusion using ADMM v2 (Hard Constraint)
%
% Optimization Problem:
% min_{s}  1/2 * || s_S - P_S * s ||^2_F + eta * || D * s ||_1
% subject to: P_A * s = s_A
%
% Where:
% s:     The target sinogram to be reconstructed (vectorized).
% s_S:   Observed spatially blurred sinogram.
% s_A:   Measured sharp sinogram samples at specific angles.
...
%
% This version uses a Hard Constraint (Projection) for the angular measurements.
...
% 2.2 Construct P_A (Angular Mask for Masking and Projection)
mask_A = zeros(M, N);
mask_A(:, 1:2:end) = 1;
...
results = []; 
fprintf('Starting Grid Search (v2 - Hard Constraint)...\n');
...
for e_idx = 1:length(eta_list)
    for m_idx = 1:length(mu_list)
        ...
        s_k = zeros(mn, 1);
        u_k = zeros(2*mn, 1);
        A_op = @(x) apply_LHS_v3(x, P_S_sub, D, mu, M, N);
        ...
        for k = 1:max_iter
            ...
            % Step 2.5: Apply Hard Constraint Projection (Sa = Pa * s)
...
s_k = zeros(mn, 1);
u_k = zeros(2*mn, 1); 
A_op_best = @(x) apply_LHS_v3(x, P_S_sub, D, best_mu, M, N);
...
subplot(1,4,1); imshow(sinogram_true, []); title('Original Sinogram');
subplot(1,4,2); imshow(S_final, []); title('Final Sinogram (Hard Cstr)');
subplot(1,4,3); imshow(X_true, []); title('Original Image');
...
function y = apply_LHS_v3(x_vec, P_S_sub, D, mu, M, N)
    X = reshape(x_vec, [M, N]);
    
    % Data fidelity term: Ps' * Ps * s
